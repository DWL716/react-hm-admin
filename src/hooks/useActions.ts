...
// hooks 一定要以use开头
function useActions(actions: IActions, deps?: DependencyList | undefined): any {
  ...
  return useMemo(() => {
    ...
    keys.forEach((key: string) => {
      ...
      if( newActions[key].length === 2 ) {
        // newActions[key] = bindPromiseCreators((actions[key]), dispatch);
      } else {
        newActions[key] = bindActionCreators((actions[key]), dispatch);
      }
    })
